...
describe("resolver", () => {
  ...
  it.for(cases)("%s", ([, context, request, expected]) => {
    const resolver = new ResolverFactory({
      modules: ["src/a", "src/b", "src/common", "node_modules"],
      extensions: [".js", ".jsx", ".ts", ".tsx"]
    });

    expect(resolver.sync(context, request).path).toBe(expected);
  });

  it("resolve pnpm package", () => {
    ...
    expect(resolver.sync(fromResolvedDir, "react")).toEqual({
      path: join(
        rootDir,
  ...
  it("resolve recursive symbol link", () => {
  ...
  it.skipIf(!!process.env.WASI_TEST)("resolve in pnp project", () => {
